...
template <typename T>
bool binary_search_tree(BinTree<T> const& tree) {
    auto it = tree.begin();
    while (it != tree.end()) {
        //Funciona si it == tree.end() - 1?
        if (*it >= *(++it)) {
            return false;
        }
    }
...
template <typename T>
void tree_sort(std::vector<T> & elems) {
    std::set<T> setElems;
    for (T el : elems) {
        setElems.insert(el);
    }
...
template <typename T>
void tree_sort_rep(std::vector<T> & elems) {
    std::map<T, size_t> mapElems;
    for (T el : elems) {
        if (!mapElems.count(el)) {
            mapElems[el] = 1;
        }
        else {
            ++mapElems[el];
    ...
    size_t i = 0;
    for (auto [key, val] : mapElems) {
        for (size_t ii = 0; ii < val; ++ii) {
            elems[i] = key; 
            ++i;
        }
    }
...
int main() {
    std::vector<int> v = {2, 123, 2, 12, 14, 1, 14};
