...
private:
  ...
  static std::pair<bool, T> lower_bound(Node *root, T const& elem) {
    if (root == nullptr) {
        return {false, elem};
    }
    else if (root->elem == elem) {
    ...
    //root->elem > elem
        ...
        std::pair<bool, T> leftSide = lower_bound(root->left, elem);
        bool exits = true;
        int number = leftSide.first? leftSide.second : root->elem;
        return {exits, number};
    }
  }
};
