...
// ================================================================
//@ <answer>
pair<bool, int> resolver(const BinTree<int> &arbol) {
    if (arbol.empty()) {
        return {true, 0};
    }
    else {
        auto [bIzq, zeroIzq] = resolver(arbol.left());
        auto [bDer, zeroDer] = resolver(arbol.right());

        int difZeros = max(zeroIzq - zeroDer, zeroDer - zeroIzq);
        bool sol = bIzq && bDer &&
                   difZeros <= 1;

        int numZeros = (arbol.root() == 0? 1 : 0);
        numZeros += zeroIzq + zeroDer;
