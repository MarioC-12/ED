...
public:
  ...
  //NEW METHOD
  void add_to(const size_t pos, const size_t sum);
...
template <typename T>
void ListLinkedDouble<T>::add_to(const size_t pos, const size_t sum) {
    auto it = begin(), otherIt = begin();
    for (size_t i = 0; i < pos; ++i) {
        ++it; ++otherIt;
    }
    int amount = *it + sum; 
    while (otherIt != end() && *otherIt < amount) {
        ++otherIt;
    }
    *it = amount;
    //Old
    ...
    it.current->next = otherIt.current;
    it.current->prev = otherIt.current->prev;
    otherIt.current->prev->next = it.current;
    otherIt.current->prev = it.current;
}
