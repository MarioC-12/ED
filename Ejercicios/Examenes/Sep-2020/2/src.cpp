...
template <typename T> 
BinTree<T> leer_arbol(T vacio) {
  T elem;
  cin >> elem;
  if (elem == vacio) {
    return {};
  } else {
    auto izq = leer_arbol(vacio);
    auto dch = leer_arbol(vacio);
    return {izq, elem, dch};
  }
}
...
pair<int, int> resolver(const BinTree<int> &tree) {
    if (tree.empty()) {
        ...
        auto [maxIzq, unoIzq] = resolver(tree.left());
        auto [maxDer, unoDer] = resolver(tree.right());

        int maxT, ambosT, unoT;
        ambosT = unoIzq + unoDer + tree.root();
        unoT = max(unoIzq, unoDer) + tree.root();
        maxT = max(ambosT, max(maxDer, maxIzq));
...
void resuelveCaso() {
    ...
    BinTree<int> tree = leer_arbol<int>(-1);

    // resolver el caso posiblemente llamando a otras funciones
    auto [sol, x] = resolver(tree);
