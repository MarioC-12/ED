...
//@ <answer>
using namespace std;
tuple<int, int, int> resolver(const BinTree<int> &tree, int depth) {
    if (tree.empty()) {
        return {0, -1, depth};
    }
    else {
        auto [sumIzq, valIzq, depIzq] = resolver(tree.left(), depth + 1);
        auto [sumDer, valDer, depDer] = resolver(tree.right(), depth + 1);

        int sumTotal = sumIzq + sumDer; 
        int valTotal; int depthSol;
        if (valIzq == -1 && valDer != -1) {
            valTotal = valDer; depthSol = depDer;
        }
        else if (valIzq != -1 && valDer == -1) {
            valTotal = valIzq; depthSol = depIzq;
        }
        else if (valIzq != -1 && valDer != -1) {
            if (depIzq >= depDer) {
                valTotal = valIzq; depthSol = depIzq;
            ...
            if (sumTotal == tree.root()) {
                valTotal = tree.root(); depthSol= depth;
            }
            else {
                valTotal = -1; depthSol= depth;
            }
        }

        return {sumTotal + tree.root(), valTotal, depthSol};
    }
}

int nodo_cuadrado_mayor_prof(const BinTree<int> &tree) {
    auto [a1, sol, a2] = resolver(tree, 1);
    return sol;
}
...
void resuelveCaso() {
    ...
    cout << nodo_cuadrado_mayor_prof(tree) << '\n';
}
