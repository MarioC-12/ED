...
//@ <answer>
// Nombre y Apellidos: Mario Calvarro Marines
//@ </answer>
...
public:
  ...
  // *** nuevo método (impleméntalo debajo, en el espacio reservado para ello)
...
private:
  ...
  //@ <answer>
  static int count_interval(Node *root, T const& lower, T const& upper) {
    if (root == nullptr) return 0;
    else if (root->elem == lower || root->elem < lower) {
        int masUno = (root->elem == lower)? 1 : 0;
        return masUno + count_interval(root->right, lower, upper);
    }
    else if (root->elem == upper || root->elem > upper) {
        int masUno = (root->elem == upper)? 1 : 0;
        return masUno + count_interval(root->left, lower, upper);
    }
    else {
        return 1 + count_interval(root->left, lower, upper) + count_interval(root->right, lower, upper);
    }
  }
  //@ </answer>
};
...
// Implementa a continuación el método pedido. Puedes declarar otros
// métodos dentro de la clase (arriba), pero implementándolos aquí debajo.
//@ <answer>
template <typename T>
